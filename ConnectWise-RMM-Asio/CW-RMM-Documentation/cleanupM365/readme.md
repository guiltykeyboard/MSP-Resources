...
```powershell
$dir=(Get-Location).Path;$confirm='https://www.microsoft.com/en-us/download/confirmation.aspx?id=49117';Write-Host "Fetching confirmation page..." -ForegroundColor Cyan;$resp=Invoke-WebRequest -UseBasicParsing -Uri $confirm -ErrorAction Stop;$m=[regex]::Match($resp.Content,'https://download\.microsoft\.com/[^\s"''>]*officedeploymenttool_[^\s"''>]*\.exe');if(-not $m.Success){Write-Warning "Could not find officedeploymenttool_*.exe link on the confirmation page."}else{$exeUrl=$m.Value;Write-Host "Downloading ODT from: $exeUrl" -ForegroundColor Yellow;$dest=Join-Path $dir 'OfficeDeploymentTool.exe';Invoke-WebRequest -UseBasicParsing -Uri $exeUrl -OutFile $dest -ErrorAction Stop;$info=Get-Item $dest;Write-Host "Saved to: $($info.FullName)" -ForegroundColor Green;Write-Host "Size: $($info.Length) bytes  Modified: $($info.LastWriteTime)"}
```

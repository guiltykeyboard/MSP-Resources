    ...
    try:
        ...
        for it in issues:
            if it.get("title") == title:
                ...
                gh_api(f"https://api.github.com/repos/{repo}/issues/{it['number']}", token, method="PATCH", payload={"body": body, "labels": ["ci", "lint"]})
                return it['number']
        # Create new issue
        created = gh_api(f"https://api.github.com/repos/{repo}/issues", token, method="POST", payload={"title": title, "body": body, "labels": ["ci", "lint"]})
